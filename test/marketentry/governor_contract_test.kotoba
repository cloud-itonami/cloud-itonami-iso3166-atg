  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Fabrication Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Fabrication Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest vendor-class-insufficient-is-held-and-unoverridable
  (testing "registered vendor Class falls short of the Class the contract value requires -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:vendor-class-insufficient} (-> (store/ledger db) last :basis)))
...
(deftest tin-unverified-is-held-and-unoverridable
  (testing "unverified TIN (Taxpayer Identification Number) when required -> HARD hold"
    (let [[db actor] (fresh)
