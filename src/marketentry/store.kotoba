(ns marketentry.store
  "SSoT for the Antigua and Barbuda (ATG) market-entry compliance actor,
...
(defn demo-data
  ...
  `:contract-value` (EC$) / `:vendor-class` are ground truth for the
  `vendor-class-insufficient` flagship check (tendersboard.gov.ag Class
  1/2/3 tiers); `:requires-tin?` / `:tin-verified?` are ground truth for
  the conditional TIN check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Fabrication Ltd" :portal "tendersboard.gov.ag"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :contract-value 750000 :vendor-class 2
             :requires-tin? true :tin-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ATG" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "tendersboard.gov.ag"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :contract-value 750000 :vendor-class 2
             :requires-tin? true :tin-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "tendersboard.gov.ag"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "ATG" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export Ltd" :portal "tendersboard.gov.ag"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :contract-value 1500000 :vendor-class 2
             :requires-tin? true :tin-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ATG" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "tendersboard.gov.ag"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :contract-value 750000 :vendor-class 2
             ...
             :drafted? false :submitted? false
             :jurisdiction "ATG" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "tendersboard.gov.ag"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 6
             :claimed-fee 550000.0
             :contract-value 25000 :vendor-class 1
             :requires-tin? true :tin-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ATG" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    (some? vendor-class)                  (assoc :engagement/vendor-class vendor-class)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/contract-value :engagement/vendor-class
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :vendor-class (:engagement/vendor-class m 0)
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-atg"})
