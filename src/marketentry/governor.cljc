(ns marketentry.governor
  ...
  notion of Antigua and Barbuda procurement law, whether a claimed
  engagement fee actually equals base + months x rate, whether the
  engagement's own declared vendor-registration Class actually meets
  what its own declared contract value requires, whether a Taxpayer
  Identification Number (TIN) has been verified for a filing that
  requires it, or when a draft stops being a draft and becomes a
  real-world tendersboard.gov.ag portal submission, so this MUST be a
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Vendor class insufficient
       for contract value            -- for `:filing/submit`,
                                       INDEPENDENTLY recompute which
                                       tendersboard.gov.ag
                                       vendor-registration Class
                                       (1/2/3) the engagement's own
                                       declared `:contract-value`
                                       requires, and HARD-hold if the
                                       engagement's own declared
                                       `:vendor-class` falls short of
                                       it. FLAGSHIP genuinely new check
                                       ...
                                       fourth distinct check SHAPE
                                       alongside Bulgaria's
                                       percentage-of-turnover formula,
                                       Albania's flat-constant
                                       threshold, and Azerbaijan's/
    ...
    5. TIN unverified               -- for `:filing/submit`, when the
                                       ...
                                       as a SEPARATE, subsequent act to
                                       ABIPCO business registration.
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(ABIPCO登録/ベンダー登録/TIN確認等)が充足していない状態での提案"}]))))

(defn- vendor-class-insufficient-violations
  "For `:filing/submit`, INDEPENDENTLY recompute which
  tendersboard.gov.ag vendor-registration Class the engagement's own
  declared `:contract-value` requires, and HARD-hold if the engagement's
  own declared `:vendor-class` falls short of it -- the flagship
  ...
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (registry/vendor-class-insufficient? e)
        [{:rule :vendor-class-insufficient
          :detail (str subject " の登録ベンダークラス(" (:vendor-class e)
                      ")が申告契約額(EC$" (:contract-value e)
                      ")の要求クラス(" (registry/required-vendor-class (:contract-value e))
                      ")に満たない -- 提出提案は進められない")}]))))
...
(defn- tin-unverified-violations
  ...
  Identification Number (TIN) the Inland Revenue Department issues as a
  separate, subsequent act to ABIPCO business registration."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (vendor-class-insufficient-violations request st)
